...
html.resize = function(key){
    switch(key){
        ...
        break;
        case constants.SECTION_DASHBOARD:
...
html.loadArea = function(area){
    ajax.run({
        data: ({module: constants.MODULE_HTML, action:area}),
...
html.load = function(page, showProgressBar, fast) {
    ajax.run({
        data: ({module: constants.MODULE_HTML, action: page}),
        fast: fast,
            ...
            $(constants.EL_OFFICE_HEAD_LOGO_MENU_ITEM).each(function() {
                $(this).click(function(){
                    if(!$(this).is(constants.EL_OFFICE_HEAD_LOGO_MENU_ITEM_LOGOUT))
                        html.load($(this).attr(constants.ATTR_CLASS), true);
                    else
                        auth.logout();
                });
            });
